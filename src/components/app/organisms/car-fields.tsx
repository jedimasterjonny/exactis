...
// third, the monthly payment and the years left on the fourth, and on a
// PCP the balloon on the last, under what becomes of it. The first four
// are uncontrolled, mount with the car as it opened and report each
// change to the dialog, whose draft mirrors them. The finance's five
// are shown by value: the balance and the balloon because the fields
// come and go with the agreement and have to come back showing what was
// typed, and the three figures because the dialog works one of them out
// from the other two, and which one changes hands as they are typed.
// The one worked out says so beneath itself. The two that can have no
// answer say so too: a rate none fits is an error, since the car cannot
// be saved without one, and a term the payment never reaches is a hint,
// since finance that never clears is paid to the end of the plan. A car
// owned outright shows no finance fields at all.
