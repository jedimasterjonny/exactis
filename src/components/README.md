...
| `organisms/` | a whole region of a screen                       | `account-dialog`, `account-fields`, `account-ledger`, `account-table`, `car-dialog`, `car-fields`, `cash-flow-card`, `employment-fields`, `expense-schedule`, `house-dialog`, `house-fields`, `income-schedule`, `line-fields`, `loan-fields`, `payment-order`, `progress-points`, `projection-chart`, `sacrifice-fields`, `schedule-rows` |
...
sixteen of them: `account-ledger` on `account-dialog`, `account-table`,
`car-dialog`, `house-dialog` and `payment-order`, `account-dialog` on
`account-fields` and `sacrifice-fields`, `car-dialog` on `car-fields`,
`house-dialog` on `house-fields`, each of `car-fields` and `house-fields` on
`loan-fields`, `income-schedule` on `employment-fields`, and each of the two
schedules on both `line-fields` and `schedule-rows`.
