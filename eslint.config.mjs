...
const eslintConfig = defineConfig([
  ...
  globalIgnores([
    ...
    "coverage/**",
    // The worktrees an agent is given inside the checkout. .gitignore keeps
    // them out of prettier, knip and oxlint, which read it; ESLint's flat
    // config reads no ignore file, so every worktree present was linted as a
    // second copy of the tree, each against a TypeScript program of its own.
    // Two of them took a 2.5-minute run past ten.
    ".claude/worktrees/**",
    // Vendored 1:1 from the shadcn registry. Linting these would demand 187
    // hand-edits that `shadcn add --overwrite` discards on the next update,
    // and every one of them is a byte the weekly drift check would report.
    // Not a hole in the gates: tsconfig still includes them, so tsc and
    // next build type check this directory like any other, and what is
    // switched off here is style rather than correctness. The kit wrappers
    // in front of them are held to everything.
    "src/components/ui/**",
  ]),
]);
